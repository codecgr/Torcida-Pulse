...
The deployed Node backend performs a five-call authenticated TxLINE chain, normalizes it into one browser-safe ReplayEnvelope, never returns raw proof blobs or credentials, and serves a clearly labeled fictional fallback after three seconds if real data is unavailable. The complete backend attempt is capped at 12 seconds and every Solana RPC request at three seconds. A dated replay manifest prevents the selected historical fixture from silently expiring.
