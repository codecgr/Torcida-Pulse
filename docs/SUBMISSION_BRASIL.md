...
É uma segunda tela mobile-first PT-BR/EN, sem apostas, para rever uma partida sem spoiler. Um toque explícito inicia o playhead de 20 s, que revela eventos de placar da TxLINE e pausa na virada factual. Nesse instante, o player libera a tela para uma história em linguagem de fã: mudança do placar, mesma tupla StablePrice antes/depois, aviso claro “pulso do jogo — não é chance de vitória” e estado verdadeiro da validateStatV2 na Solana devnet. O compartilhamento envia a história factual, guardar é reversível e o player final não cobre a memória. Não há aposta, custódia, trade, exigência de wallet ou recomendação financeira.

O backend Node faz cinco chamadas reais autenticadas: (1) GET /api/fixtures/snapshot?startEpochDay=20649; (2) GET /api/scores/historical/18241006; (3) GET /api/odds/snapshot/18241006?asOf=<timestamp da virada - 120000 ms>; (4) o mesmo endpoint com <timestamp + 120000 ms>; (5) GET /api/scores/stat-validation?fixtureId=18241006&seq=871&statKeys=1,2. Ele normaliza tudo em um ReplayEnvelope sem expor token, JWT, payload bruto ou proof blob. Estados esperados de dados reais protegidos/desligados oferecem uma demo pública rotulada em um toque; uma chamada real ainda em andamento oferece a mesma rota após 3 s. Deadline total: 12 s; RPC abortável: 3 s; timeout de odds preserva timeline; timeout de prova vira indisponível. Um manifesto datado exige rotação antes da janela histórica expirar.
